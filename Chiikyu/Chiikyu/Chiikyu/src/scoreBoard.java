...
public class scoreBoard {
	...
	public void showScore() throws IOException {
		...
		for (int i = 0; i < Math.min(10, counter); ++i) {
			bufferWritter.write(scoreList.get(i).score + " "
					+ scoreList.get(i).name + " "
					+ scoreList.get(i).time.getTime() + "\r\n");
			String tempDate = new String();
			tempDate = (new SimpleDateFormat("yyyy-MM-dd HH:mm:ss"))
					.format(scoreList.get(i).date.getTime());
			tempDate = (i + 1) + "   " + tempDate + "   "
					+ scoreList.get(i).score + "   " + scoreList.get(i).name;
	...
	private boolean clearData(boolean showInfo) throws IOException {
		if (showInfo) {
			if (JOptionPane.showConfirmDialog(null,
					"Are you sure you want to clear the score board?", "Info",
					JOptionPane.YES_NO_OPTION) == 0) {
				if (filePointer.delete()) {
					JOptionPane.showMessageDialog(null, "Success!", "Info",
							JOptionPane.INFORMATION_MESSAGE);
					listModel.clear();
					return true;
				} else {
					JOptionPane.showMessageDialog(null, "Failed.", "Info",
			...
			listModel.clear();
			if (!filePointer.delete()) {
				JOptionPane.showMessageDialog(null, "Failed clearing data.",
